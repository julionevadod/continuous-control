...
class Actor(nn.Module):
    def __init__(self, input_size: int, output_size: int, fc1_size: int = 400, fc2_size: int = 300):
        super().__init__()

        self.output_size = output_size

        self.net = nn.Sequential(
            nn.Linear(input_size, fc1_size, dtype=torch.float32),
            nn.ReLU(),
            nn.Linear(fc1_size, fc2_size, dtype=torch.float32),
            nn.ReLU(),
            nn.Linear(fc2_size, output_size, dtype=torch.float32),
            nn.Tanh(),
        )
